...
void *map_insert(struct map_t *map, void *key, size_t key_size, void *value) {
    ...
    if (((int)(map->length)/(int)(map->capacity)) > 0.7 ){
        int new_cap = map->capacity *= 2;
        struct node **new_hashtable = calloc(new_cap, sizeof(h_node *));
        for (size_t i = 0; i < map->capacity; i++){
            if (map->hashtables[i]){
                uint64_t hashed_key = map->hashfn(map->hashtables[i]->key);
                long long unsigned hashed_index = hashed_key % new_cap;
                new_hashtable[hashed_index] = map->hashtables[i];
            }
        }
        map->capacity = new_cap;
        free(map->hashtables);
        map->hashtables = new_hashtable;
    }
